    ...
    datasets: [
      {
        ...
        borderColor: 'rgba(255, 180, 171, 0.8)',
        borderWidth: 1,
        borderRadius: 3,
      },
      {
        ...
        backgroundColor: 'rgba(0, 219, 233, 0.5)',
    ...
    <div className="space-y-8">
      <header>
        ...
        <p style={{ color: '#849495', fontFamily: 'Space Grotesk', fontSize: '13px', letterSpacing: '0.05em', marginTop: '4px' }}>
          REAL-TIME MONITORING — AIIMS | FORTIS | APOLLO | MEDANTA | MAX
        </p>
      </header>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'ICU_BEDS', value: totalICU, color: '#ffb4ab', icon: 'bed' },
          { label: 'GENERAL_BEDS', value: totalGeneral, color: '#00dbe9', icon: 'hotel' },
          { label: 'O2_CYLINDERS', value: totalO2, color: '#bcff5f', icon: 'air' },
          { label: 'VENTILATORS', value: totalVent, color: '#aec6ff', icon: 'pulmonology' }
        ].map(stat => (
          <div key={stat.label} className="glass-panel p-5 tech-grid relative overflow-hidden">
            <div className="absolute top-3 right-3">
              <span className="material-symbols-outlined" style={{ color: stat.color, opacity: 0.2, fontSize: '28px' }}>{stat.icon}</span>
            </div>
            ...
            <p style={{ fontSize: '10px', color: '#3b494b', fontFamily: 'Space Grotesk', marginTop: '2px' }}>{hospitals.length} NODES_ACTIVE</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div style={{ height: '320px' }}>
            <Bar 
              ...
              options={{
                ...
                scales: {
                  y: { 
                    ticks: { color: '#3b494b', font: { family: 'Space Grotesk', size: 11 } }, 
                    grid: { color: 'rgba(0, 219, 233, 0.05)' } 
                  },
                  x: { 
                    ticks: { color: '#3b494b', font: { family: 'Space Grotesk', size: 10 }, maxRotation: 45 }, 
                    grid: { color: 'rgba(0, 219, 233, 0.05)' } 
                  }
                }
              }} 
        ...
        <div className="glass-panel p-6 flex flex-col" style={{ height: '26rem' }}>
          ...
          <div className="overflow-y-auto flex-1 pr-2 space-y-3">
              ...
              <div key={p._id} className="p-4 rounded flex justify-between items-center transition hover:bg-white/[0.02]" style={{ background: 'rgba(30, 32, 36, 0.5)', border: '1px solid rgba(59, 73, 75, 0.3)' }}>
                <div>
                  <h3 style={{ fontFamily: 'Space Grotesk', fontWeight: 600, fontSize: '15px', color: '#e2e2e8' }}>
                    {p.name} <span style={{ color: '#849495', fontSize: '12px', fontWeight: 400 }}>({p.age} yrs)</span>
                  </h3>
                  <p style={{ fontSize: '12px', color: '#849495', marginTop: '2px' }}>STATUS: <span style={{ color: '#e2e2e8' }}>{p.status?.toUpperCase()}</span></p>
                  <p style={{ fontSize: '12px', color: '#00dbe9', marginTop: '2px' }}>
                    ...
                    {p.allocated_hospital ? p.allocated_hospital.name : 'UNALLOCATED'}
                  </p>
                </div>
            ...
            {patients.length === 0 && <p style={{ color: '#3b494b', textAlign: 'center', padding: '2rem 0', fontFamily: 'Space Grotesk' }}>NO_PATIENTS_ALLOCATED</p>}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {hospitals.map(h => (
          <div key={h._id} className="glass-panel p-6 tech-grid relative overflow-hidden hover:-translate-y-0.5 transition-transform">
            ...
            <h3 className="neon-text mb-4" style={{ fontFamily: 'Space Grotesk', fontSize: '16px', fontWeight: 700, letterSpacing: '-0.01em' }}>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { label: 'GENERAL', value: h.resources.general_beds, color: '#00dbe9' },
                { label: 'O2', value: h.resources.oxygen_cylinders, color: '#bcff5f' },
                { label: 'VENTILATORS', value: h.resources.ventilators, color: '#aec6ff' }
              ].map(r => (
            ...
            <div className="mt-3 pt-3 flex items-center justify-between" style={{ borderTop: '1px solid rgba(59, 73, 75, 0.2)' }}>
              <span style={{ fontSize: '10px', color: '#3b494b', fontFamily: 'Space Grotesk' }}>
                ...
                AMBULANCES: <span style={{ color: '#e2e2e8' }}>{h.resources.ambulances}</span>
              </span>
